...
const imageSrc = "assets/img/icon/6.png"; // Icon image path
...
function revealImage(button) {
  if (!button.classList.contains("clicked")) {
    ...
    setTimeout(() => {
      // Delay adding 'clicked' class to allow image insertion
...
function checkWin() {
  ...
  if (clickedButtons.length === 23) {
    // If 23 buttons are clicked, only one block is left
    gameEnded = true;
    const video = document.createElement("video");
